...
for installer in debian ubuntu; do
    ...
    sed -i 's/insert-line2/curl -s "https:\/\/beta-apt\.hestiacp\.com\/pubkey\.gpg" | gpg --dearmor | tee \/usr\/share\/keyrings\/hestia-beta-keyring\.gpg  >\/dev\/null 2>\&1/g' hst-install-$installer.sh
done
